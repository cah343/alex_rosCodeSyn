...
front_delta = 15/2.3561899662*0.524276316166
side_ang    = 30/2.3561899662*0.524276316166
side_delta  = 20/2.3561899662*0.524276316166
side_thresh = 0.875/2.3561899662*0.524276316166
...
def Callback(data):
    ...
    print data.angle_min, data.angle_max, data.angle_increment, zeroAng,leftAng, rightAng,sideOffset,zeroOffset, len(data.ranges)
    #-0.521567881107 0.524276316166 0.00163668883033 318 637 -1 213 159 640
...
def setup():
    ...
    # subscribe to all
    rospy.Subscriber("/scan", LaserScan, Callback) # (sensor_msgs/LaserScan)
    # rate = rospy.Rate(user_rate)
    rate = rospy.Rate(100)

    # publish to cmd_vel of the jackal
    pub = rospy.Publisher("/cmd_vel_mux/input/navi", Twist, queue_size=10) # (Twist)
